...
- При записи используется локальный `asyncio.Lock` по ключу `(LRP, order, business_day)`, чтобы два оператора не получили одинаковый номер в одном процессе бота.
...
- 2026-02-17: в `bot.py` добавлен локальный lock для атомарной нумерации бобин при параллельной работе операторов (`LRP + заказ + бизнес-дата`), расчет next number и запись строки выполняются в одном критическом блоке.
